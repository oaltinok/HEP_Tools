...
double PC_DST::calcCalorimetricEnergy()
{
    int subdet = 0;
    int ndigits = 0;
    double pe_c = 0;
    double evis_c = 0;
    double evis_target = 0.0;
    double evis_trkr_side_X = 0.0;
    double evis_trkr_side_UV = 0.0;
    double evis_trkr_central = 0.0;
    double evis_ecal = 0.0;
    double evis_hcal = 0.0;

    // Loop Over All ID Clusters and Get Visible Energy in Each Subdetector
    for (int i = 0;  i < n_clusters_id; i++){
        subdet = clus_id_subdet[i];
        evis_c = clus_id_energy[i]; 
        pe_c = clus_id_pe[i];
        ndigits = clus_id_size[i];

        // Don't include low charge clusters
        if ( pe_c / ndigits <= 3) continue;
        
        // Get Energy in Target
        if (subdet == 1) evis_target = evis_target + evis_c; 
        
        // Get Energy in ECAL and HCAL
        if (subdet == 4) evis_ecal = evis_ecal + evis_c;
        if (subdet == 5) evis_hcal = evis_hcal + evis_c;
        
        // Get Energy in Tracker
        if (subdet == 3){
            int strip = clus_id_strip[i];
            
            if (strip > 10 && strip < 118){
                // Energy in Central Tracker
                evis_trkr_central = evis_trkr_central + evis_c;
            }else{
                // Energy in Side ECAL 
                int view = clus_id_view[i];
                if (view == 1){
                    evis_trkr_side_X = evis_trkr_side_X + evis_c;
                }else{
                    evis_trkr_side_UV = evis_trkr_side_UV + evis_c;
                }
            }
        }
    }
    ...
    /*
     *  Don't include following events:
     *      Hits in Target Region
     *      Hits in OD
     */
...
double PC_DST::getODEvis()
{
    double od_E = 0.0;
    for (int i = 0; i < n_clusters_od; i++){
        double evis_c = clus_od_energy[i];
        
        // Don't include low activity clusters
        if (evis_c < low_activity_E) continue;
...
void PC_DST::InitHistograms()
{
    ...
    reco_true_energy->GetYaxis()->SetTitle("E_{True}");
